...
Computers are a tool that has transformed the way we work from storing information, better processes to work with that data and allowing us to easily share the data.
...
This leads to a race to make computers more friendly to the consumer by reducing their size as the years go by.
...
Let's explore first what is the classical computer to get an understanding of why quantum computers were conceived.
...
A classical <a href="https://en.wikipedia.org/wiki/Computer" rel="noopener" target="_blank">computer</a> or just a computer
...
The CPU is one of the pieces that take complicated data and decodes it into binary.
...
The reason why the "on" state is represented by a negative charge is that electrons are represented by a negative charge.
More electrons mean more current with a negative charge. In binary, we represent the "on" state with a 1 and the "of" state with a 0.
The early computers were way bulkier than what we have now but we've kept the same fundamental principles.
...
A <a href="https://en.wikipedia.org/wiki/Transistor" rel="noopener" target="_blank">Transistor</a> is a semiconductor device used to amplify or switch electron signals and electrical power.
It's composed of semiconductor material usually with at least three terminals to connect to an external circuit.
Transistors act like a switch turning on and off the current. This is where binary comes in.
The transistor is a fundamental building block for all electronics making devices smaller and cheaper to manufacture revolutionizing the world of electronics paving the way for smaller computers.
...
Modern CPUs contain millions of transistors that are microscopic.
...
<a href="https://en.wikipedia.org/wiki/Moore%27s_law" rel="noopener" target="_blank">Moore's Law</a> is the observation that the number of transistors in an integrated circuit doubles every two years.
...
Similar to the bit in a classical computer a qubit will result in a 1 or a 0 but the general state according to quantum mechanics can be a superposition of both.
...
By using physics we can frame these sorts of problems as energy minimization problems.
...
As Moore's law keeps getting slower consumers will keep asking for faster devices and this could accelerate the development of quantum computers.
