...
This method is a bit difficult to understand at first but nothing worth doing is easy. Reduce uses an accumulator that will be the total of the calculation you are doing in the reduce method. It also uses current value that the previous methods use. Now let's see the parameters that reduce takes and an example of usage.
